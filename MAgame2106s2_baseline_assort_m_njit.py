...
# trains pd strategies using q-learning and records cooperation, policies and agent behaviour
...
def run_simulation(
    ...
    agent_count_switch_t = np.zeros((training_horizon, number_agents), dtype=np.int64)
    agent_count_defection_t = np.zeros((training_horizon, number_agents), dtype=np.int64)
    ...
    for training_step in range(1, training_horizon + 1):
        ...
        for game_round in range(rounds_per_game):
            groups = make_baseline_groups(number_agents, actions_pd, m)
...
def run_experiment_batch(
    ...
    if tStart != 0:
        raise NotImplementedError("tStart must be 0.")

    result_directory = (
        f"result_{gameName}_{algoName}_lr{lr:.2f}_Npast1"
        f"_Nagent{Nagent}_R{roundsG}_baseline_m{m:.2f}"
    )
    ...
    for simulation in range(simStart, Nsim + 1):
        ...
        print(
            f"Completed: baseline, sim={simulation}, m={m:.2f}"
        )
