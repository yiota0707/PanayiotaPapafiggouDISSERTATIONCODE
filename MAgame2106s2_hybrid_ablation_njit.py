# implements the hybrid reinforcement-learning model with learned partner stay/switch decisions
# combines partner switching, assortative rematching and pd learning while recording behavioural outcomes
...
# apply learned stay switch decisions
...
def rematch_agents(
    ...
    if switch_pool_size > 0:
        ...
        if np.random.random() < m:
            groups_switch = create_assortative_pairs(
                switching_agents,
                actions_pd,
            )
        else:
            groups_switch = create_random_pairs_from_ids(
                switching_agents
            )
...
def run_experiment_batch(
    ...
    result_directory = (
        ...
        f"_Nagent{Nagent}_R{roundsG}_hybrid_m{m:.2f}_gamma{gamma:.2f}"
    )
